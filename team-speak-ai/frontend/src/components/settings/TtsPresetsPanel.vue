<!--
  TtsPresetsPanel — TTS 预设管理（平台 + 模型）
  左侧平台列表，右侧模型列表，弹窗编辑。
-->
<template>
  <div class="lpp-root">
        ...
        <div v-for="m in selectedPlatform.models" :key="m.id" class="lpp-model-card">
          <div class="lpp-mc-main">
            ...
            <div class="lpp-mc-tags">
              <span class="lpp-tag">{{ m.voice_id }}</span>
              <span class="lpp-tag">speed: {{ m.speed ?? 1 }}</span>
              <span class="lpp-tag">{{ m.streaming !== false ? '流式' : '非流式' }}</span>
              <span class="lpp-tag" v-if="m.emotion">{{ m.emotion }}</span>
            </div>
          </div>
    ...
    <div v-if="platformModal.visible" class="lpp-overlay" @click.self="platformModal.visible = false">
      <div class="lpp-modal">
        ...
        <div class="lpp-field">
          <label>平台名称</label>
          <input v-model="platformModal.form.name" placeholder="例如: MiniMax TTS 平台" />
        </div>
        <div class="lpp-field">
        ...
        <div class="lpp-field" v-if="platformModal.form.provider !== 'edge'">
          <label>API Key</label>
      ...
      <div class="lpp-modal lpp-modal-wide">
        ...
        <div class="lpp-row">
          <div class="lpp-field lpp-col">
            <label>模型名称</label>
            <input v-model="modelModal.form.name" placeholder="例如: speech-2.8-hd" />
          </div>
          <div class="lpp-field lpp-col">
        ...
        <div class="lpp-section-title">音色设置</div>
        <div class="lpp-row">
          <div class="lpp-field lpp-col">
            <label>Voice ID</label>
            <input v-model="modelModal.form.voice_id" placeholder="male-qn-qingse" />
          </div>
        </div>

        <div class="lpp-row">
          <div class="lpp-field lpp-col">
            <label>语速 ({{ modelModal.form.speed ?? 1 }})</label>
            <input type="range" v-model.number="modelModal.form.speed" min="0.5" max="2" step="0.1" />
          </div>
          <div class="lpp-field lpp-col">
            <label>音量 ({{ modelModal.form.vol ?? 1 }})</label>
            <input type="range" v-model.number="modelModal.form.vol" min="0.1" max="10" step="0.1" />
          </div>
        </div>
        <div class="lpp-row">
          <div class="lpp-field lpp-col">
            <label>语调 ({{ modelModal.form.pitch ?? 0 }})</label>
            <input type="range" v-model.number="modelModal.form.pitch" min="-12" max="12" step="1" />
          </div>
          <div class="lpp-field lpp-col">
            <label>情绪</label>
            <select v-model="modelModal.form.emotion">
        ...
        <div class="lpp-section-title">音频设置</div>
        <div class="lpp-row">
          <div class="lpp-field lpp-col">
            <label>采样率</label>
          ...
          <div class="lpp-field lpp-col">
            <label>比特率</label>
            <select v-model.number="modelModal.form.bitrate">
        ...
        <div class="lpp-row">
          <div class="lpp-field lpp-col">
            <label>格式</label>
            <select v-model="modelModal.form.format">
          ...
          <div class="lpp-field lpp-col">
            <label>声道</label>
            <select v-model.number="modelModal.form.channel">
        ...
        <div class="lpp-row">
          ...
          <div class="lpp-field lpp-col">
            <label>语言增强</label>
            <input v-model="modelModal.form.language_boost" placeholder="留空=自动" />
          </div>
        </div>
...
async function testModel(platformId, modelId) {
  ...
  try {
    const resp = await pipelineSocket.sendCommand('_system', 'tts_preset.test', { platform_id: platformId, model_id: modelId })
...
const platforms = computed(() => store.ttsPlatforms)
...
// ── 平台弹窗 ──
const emptyPlatform = () => ({ name: '', provider: 'minimax', api_key: '' })
...
async function onSaveModel() {
  ...
  await store.saveTtsModel(selectedPlatform.value.id, payload)
  modelModal.value.visible = false
}

async function onDeleteModel(platformId, modelId) {
  if (!confirm('确认删除此模型？')) return
  await store.deleteTtsModel(platformId, modelId)
}

async function onDuplicateModel(platformId, modelId) {
  await store.duplicateTtsModel(platformId, modelId)
}

function onRefresh() {
  pipelineSocket.sendCommand('_system', 'tts_preset.list', {})
}
...
.lpp-field input[type="range"] { padding: 4px 0; }
