...
function getPortState(portId) {
  const nc = editorStore.connections.some((c) =>
    (c.from_node === props.node.id && c.from_port === portId) ||
    (c.to_node === props.node.id && c.to_port === portId))
  if (!nc) return 'disconnected'
  return executionStore.getNodeStatus(props.node.id).status === 'processing' ? 'flowing' : 'connected'
}

const nodeStatus = computed(() => executionStore.getNodeStatus(props.node.id).status || 'pending')
const isProcessing = computed(() => nodeStatus.value === 'processing')
const isListening = computed(() => nodeStatus.value === 'listening')
const progressPercent = computed(() => { const p = executionStore.getNodeStatus(props.node.id).progress; return p ? Math.round(p * 100) : 0 })

const statusClass = computed(() => {
  const m = { pending: 'status-pending', processing: 'status-processing', completed: 'status-completed', error: 'status-error', listening: 'status-listening' }
  return m[nodeStatus.value] || 'status-pending'
})
const statusLabel = computed(() => {
  const m = { pending: '待命', processing: '处理中', completed: '已完成', error: '错误', listening: '监听中' }
  return m[nodeStatus.value] || nodeStatus.value
})
const borderClass = computed(() => {
  const m = { pending: 'border-outline', processing: 'border-primary node-pulse', completed: 'border-secondary', error: 'border-error', listening: 'border-primary' }
  return m[nodeStatus.value] || 'border-outline'
})
const badgeColor = computed(() => {
  const c = { primary: '#adc7ff', secondary: '#4edea3', tertiary: '#ef6719', outline: '#8b90a0' }
...
function onDragStart(e) {
  ...
  const cx = props.node.position.x
  const cy = props.node.position.y
  const sx = e.clientX
  const sy = e.clientY
  ...
  function onUp(ev) {
    ...
    if (nodeDragMoved) {
      const dx = (ev.clientX - sx) / z
      const dy = (ev.clientY - sy) / z
      editorStore.moveNodeLocal(props.node.id, Math.round(cx + dx), Math.round(cy + dy))
...
.node-card {
  ...
  border-radius: 8px; z-index: 10; cursor: pointer;
  transition: transform 0.2s, box-shadow 0.2s, border-color 0.2s;
  border: 1px solid; user-select: none;
}
.node-card:hover { transform: translateY(-2px); z-index: 20 !important; }
.node-card.selected { border-color: #4a8eff !important; box-shadow: 0 0 16px rgba(74,142,255,0.3); }
...
.keyword-dot { width: 8px; height: 8px; border-radius: 50%; background: #ef6719; margin-left: 4px; animation: keywordPulse 1.5s ease-in-out infinite; }
...
.tab-btn { position: relative; padding: 6px 12px; font-size: 11px; font-family: 'Space Grotesk',sans-serif; letter-spacing: 0.05em; font-weight: 500; color: #8b90a0; cursor: pointer; transition: color 0.2s; border: none; border-bottom: 2px solid transparent; background: transparent; }
.tab-btn:hover { color: #c1c6d7; } .tab-btn.active { color: #adc7ff; border-bottom-color: #adc7ff; }
.node-body { padding: 10px; }
.node-default-body { display: flex; flex-direction: column; gap: 8px; }
.node-status-row { display: flex; justify-content: space-between; align-items: center; font-size: 12px; }
.node-status-label { color: #8b90a0; } .node-status-value { display: flex; align-items: center; gap: 4px; }
.status-dot { width: 6px; height: 6px; border-radius: 50%; }
.status-pending .status-dot { background: #8b90a0; }
.status-processing .status-dot { background: #adc7ff; box-shadow: 0 0 8px rgba(173,199,255,0.6); animation: pulse 1.5s infinite; }
.status-completed .status-dot { background: #4edea3; box-shadow: 0 0 8px rgba(78,222,163,0.6); }
.status-error .status-dot { background: #ffb4ab; box-shadow: 0 0 8px rgba(255,180,171,0.6); }
.status-listening .status-dot { background: #adc7ff; box-shadow: 0 0 8px rgba(173,199,255,0.6); animation: pulse 1.5s infinite; }
@keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.4; } }
.progress-bar { width: 100%; background: #31353d; border-radius: 9999px; height: 4px; }
.progress-fill { background: #adc7ff; height: 4px; border-radius: 9999px; transition: width 0.3s; }
.workflow-badge { position: absolute; top: -10px; right: -10px; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: bold; font-family: 'Space Grotesk',sans-serif; z-index: 40; background: #10131b; border: 2px solid; }
</style>
